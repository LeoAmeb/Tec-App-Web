        ...
        <form @submit.prevent="updateUser" v-if="loaded">
          ...
          <router-link to="/users">< Back to users</router-link>
            ...
            <div class="form-group">
                  ...
                  <input type="text" v-model="form.password"  maxlength="255" ></input>
                  <has-error :form="form" field="password"></has-error>
            </div>
            <div class="form-group">
                  <label>role_id</label>
                  <input type="number" v-model="form.role_id"></input>
            </div>
            <div class="form-group">
...
export default {
  ...
  methods: {
    getUser: function(User){
      ...
      var that = this;
      this.form.get('/api/users/'+this.$route.params.id).then(function(response){
    ...
    updateUser: function(){
      ...
      var that = this;
      this.form.put('/api/users/'+this.$route.params.id).then(function(response){
        that.form.fill(response.data);
      })
    ...
    deleteUser: function(){
      ...
      var that = this;
      this.form.delete('/api/users/'+this.$route.params.id).then(function(response){
        that.form.fill(response.data);
